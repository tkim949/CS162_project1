...
int main() {
    ...
    if(choiceResult == 1) {
        ...
        do {
            ...
            std::cout <<"Please choose a board size, input Length number with a positive integer number (for column): "<< std::endl;
            std::getline(std::cin, length);
            lengthCol = validCh(length);

            std::cout <<"Input Width number here with a positive integer number (for row): "<< std::endl;
            std::getline(std::cin, width);
            widthRow = validCh(width);
			
			/********************************************************************************************
			* *I put below two lines only here because two many line clears caused key board delay.
			* *These two line can clear garbage input data, but if users make always correct inputs,
			* *it might cause key board delay or have user click enter key again.
			*********************************************************************************************/
              std::cin.clear();
              std::cin.ignore(INT_MAX, '\n');
            ...
            if (choiceForRanNum == 1) {
				...
				std::cout<<"Your start point from the random choice: ["<<startRow<<" : "<<startCol<<"]"<<std::endl;
				...
				std::cout<<"First, Input your Row number here: "<<std::endl;
                std::getline(std::cin, userRow);
                startRow = validCh(userRow)-1;
               
                std::cout <<"Second, Input starting Column here: " << std::endl;                
                ...
                if (!((startRow < widthRow) && (startCol < lengthCol))){
                    ...
                    while (!((startRow < widthRow) && (startCol < lengthCol))) {
                        std::cout <<"Wrong input! Input within the board size!"<< std::endl;                 
                        std::cout <<"Please try again. Input start Row number first: " << std::endl;                       
                        std::getline(std::cin, userRow);
                        startRow = validCh(userRow)-1;
                        std::cout <<"Then, start Column number here: "<<std::endl;
                        std::getline(std::cin, userCol);
                        startCol =validCh(userCol)-1;
                    }
                }
